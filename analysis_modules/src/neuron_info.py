...
neuron_list = (
    ...
    'CAN', 'CEPsh', 'GLR',
    'RICRa','RICRp','PVWL_or_R_1','PVWL_or_R_2','PVWL_or_R_3',
)

def class_members(cls):
    if cls in (
        'AVG', 'DVC', 'PVR', 'PVT', 'RIH', 'RIR', 'DVA', 'AQR', 'AVM', 'PQR',
        'PVM', 'DVB', 'PDA', 'PDB', 'ALA', 'AVL', 'RID', 'RIS', 'PVW', 'SABD',
    ):
        return [cls]
    if cls in (
...
def pair_members(cls):
    if cls in (
        'AVG', 'DVC', 'PVR', 'PVT', 'RIH', 'RIR', 'DVA', 'AQR', 'AVM',
        'PQR', 'PVM', 'DVB', 'PDA', 'PDB', 'ALA', 'AVL', 'RID', 'RIS', 
        'PVW', 'SABD', 'RMEV', 'RMED'
    ):
        return [cls]
    ...
    if cls in (
        ...
        'GLRV', 'OLQV', 'OLQD', 'URAV', 'URAD', 'URYV', 'URYD', 'SMBV', 
        'SMBD', 'SMDV', 'SMDD', 'SIBV', 'SIBD', 'SIAV', 'SIAD', 'CEPV', 'CEPD',
        'SAAD', 'SAAV'
    ):
        return [cls + n for n in ('L', 'R')]
    if cls in (
        'IL1L/R', 'IL2L/R', 'RMDL/R', 'GLRL/R', 'RMEL/R'
    ):
        return [cls[:3] + n for n in ('L', 'R')]
...
def nclass(n):
    ...
    if n in ('RICRa','RICRp'):
        return 'RIC'
    if n in ('PVWL_or_R_1','PVWL_or_R_2','PVWL_or_R_3','PVWL-2'):
        return 'PVW'
    return n
...
def npair(n):
    ...
    if n in ('RICRa', 'RICRp'):
        return 'RIC'
    if n in ('PVWL_or_R_1', 'PVWL_or_R_2', 'PVWL_or_R_3'):
        return 'PVW'
